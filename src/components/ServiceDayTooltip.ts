...
export class ServiceDayTooltip extends Component {
  ...
  public override render() {
    ...
    return html`
      <div
        class="absolute -inset-x-2 top-full z-50 rounded-xl bg-neutral-800 shadow-lg ring-1 ring-white/10 ring-inset group-[:not(:hover):not(:focus-visible):not(:has(:focus-visible))]/bar:sr-only"
      >
        <div
              ...
              ${this.notices.map((n) => {
                ...
                return html`
                  <li class="relative flex items-center gap-2 py-2">
                    ...
                    <a href="/notice/${n.id}" class="font-medium text-white">${n
                      .name}<span
